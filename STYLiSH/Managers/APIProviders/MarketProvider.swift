...
class MarketProvider {
    ...
    func fetchHots(completion: @escaping PromotionHanlder) {
        if UserDataManager.shared.style == "A" {
            request = STMarketRequest.styleA
        } else if UserDataManager.shared.style == "B" {
            request = STMarketRequest.styleB
        } else if UserDataManager.shared.style == "C" {
            request = STMarketRequest.styleC
        }
        
        guard let request = request else { fatalError("Cannot create hots request") }
